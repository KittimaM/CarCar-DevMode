...
const StaffPage = ({ data }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-base-100">
      ...
      {/* Header */}
      <div className="px-8 py-8 shadow-md bg-gradient-to-r from-secondary/10 via-secondary/5 to-transparent border-b border-base-200">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-secondary uppercase tracking-wider mb-2">{labelValue}</p>
            <h1 className="text-3xl font-bold text-base-content">
              {viewMode === "list" && "Staff List"}
              {viewMode === "add" && "Add Staff"}
              {viewMode === "edit" && "Edit Staff"}
            </h1>
          </div>
          ...
          <div className="max-w-6xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              ...
              <p className="text-sm text-base-content/60">
                {filteredUsers.length} {filteredUsers.length === 1 ? "staff" : "staff members"} found
              </p>
            </div>
                    ...
                    {filteredUsers.length > 0 ? (
                      filteredUsers.map((u) => (
                        <tr key={u.id} className="hover:bg-base-50">
                          ...
                          <td>
                            <span className="badge badge-ghost badge-sm">{u.role_name}</span>
                          </td>
                      ...
                      <tr>
                        ...
                        >
                          {searchTerm ? "No staff match your search" : "No staff available"}
                        </td>
                      </tr>
                    )}
                  </tbody>
