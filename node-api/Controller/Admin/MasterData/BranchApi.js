...
const GetAllBranch = (req, res, next) => {
  Conn.execute("SELECT * FROM branch", function (error, results) {
    if (error) {
...
const PostAddBranch = (req, res, next) => {
...
const PutUpdateBranch = (req, res, next) => {
  const { id, name, address, phone } = req.body;
  Conn.execute(
    "UPDATE branch SET name = ?, address = ?, phone = ? WHERE id = ?",
